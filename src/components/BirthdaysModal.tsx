        ...
        <div className="overflow-y-auto p-4 flex flex-col gap-4">
          {characters && characters.length > 0 ? (
            characters.map((character, i) => (
              <div 
                key={i} 
                className="border border-stone-200 rounded-xl p-4 bg-white shadow-sm"
              >
                <div className="flex items-center gap-3 mb-4">
                  {/* Avatar */}
                  <div className={clsx(
                    "w-12 h-12 rounded-full flex items-center justify-center text-lg font-bold border shadow-sm shrink-0",
                    colors.secondary,
                    colors.primary,
                    colors.accent
                  )}>
                ...
                <div className="space-y-4 text-sm">
                  {/* Likes */}
                  <div>
                    <span className="text-xs font-bold text-stone-400 uppercase tracking-wider mb-1 block">
                      Loves & Likes
                    </span>
                    <div className="flex flex-wrap gap-1.5">
                      {character.likes.map((like, i) => (
                        <span 
                          key={i}
                          className="px-2 py-1 bg-rose-50 text-rose-700 rounded-md border border-rose-100 text-xs font-medium"
                        >
                          {like}
                        </span>
                      ))}
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {character.dislikes.map((dislike, i) => (
                        <span 
                          key={i}
                          className="px-2 py-1 bg-stone-100 text-stone-600 rounded-md border border-stone-200 text-xs font-medium"
                        >
                          {dislike}
                        </span>
                      ))}
